...
const cfu = {
	...
	"formatter": {
		...
		"pieWrap": function(val, index, series, opts) {
			if (index !== undefined) {
				return series[index].name  + ((series[index].value / (series[index].hotSum || series[index]
						.coldSum)) * 100)
					.toFixed(0) + '%' + "<br>" + '(' + series[index].value + '次)'
			}
		},
		"pieNumberWrap": function(val, index, series, opts) {
			if (index !== undefined) {
				return series[index].name  + "<br>" + ((series[index].value / (series[index].hotSum || series[index]
						.coldSum)) * 100)
					.toFixed(0) + '%' + '(' + series[index].value + '次)'
			}
		},
	},

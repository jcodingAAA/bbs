...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  for (let i = 0; i < seriesConvert.length; i++) {
	...
	if(item.text.includes("<br>")) {
		textList = item.text.split("<br>")
		textWidth = measureText(textList[0], item.textSize * opts.pix || config.fontSize, context);
	}else {
		textWidth = measureText(item.text, item.textSize * opts.pix || config.fontSize, context);
	}
    let startY = orginY3;
  ...
  for (let i = 0; i < textObjectCollection.length; i++) {
	...
	if(item.text.includes("<br>")) {
		...
		for (let j = 0; j<textList.length; j++) {
			context.fillText(textList[j], textStartX, (textPosition.y + j*15 + 3));
		}
	}else {

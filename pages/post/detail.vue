<template>
	<view class="index-con" :style="{paddingTop:navbarHeight +'px'}">
		...
		<view class="info-box lf-top">
			<view v-if="showType=='1'&&postDetail.media&&postDetail.media.length>0&&postDetail.type == 1">
				<u-swiper :list="postDetail.media" height="980" indicator-pos="bottomRight" :effect3d="true"
					@click="showImage"></u-swiper>
			</view>
		...
		<view class="comment-tool">
			<view style="height: 60rpx;width: 100%;display: flex;flex-direction: row;align-items: center;" @click="showComment = true">
				<view class=""
		...
		<u-popup v-model="showComment" mode="center" @close="commentClose()">
			<view class="comment-body">
				...
				<view class="contant">
					<u-input v-model="form.content" :type="'textarea'" :border="true" :height="500" :auto-height="true" />
					...
					.then(res => {
						if (res.code == 500) {
							...
							if (res.result.type == 2) {
								var res = this.$f.decryptUrl(res.result.media[0]);
								var list = JSON.parse(res)
								this.postDetail.media[0] = list[0]
							}
							this.$loading(false);
							// 投票帖子
							if (res.result.voteId && res.result.voteId > 0) {

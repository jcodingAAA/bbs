<template>
	<view>
		...
		<view v-if="pageCurrent == 1">
			...
			<!-- 帖子热榜 -->
			<post-recommend title="热门贴" desc="一周排行榜" :hotPost="hotPost"></post-recommend>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getDiscussList() {
				// this.$H.get('discuss/discussList').then(res => {
				// 	this.discussList = res.result;
				// 	let result = []
				// 	let colorArray = ["red", "grey", "blue", "orange", "yellowgreen", "purple", "brown"];
				// 	for (let i = 0; i < this.discussList.length; i++) {
				// 		let obj = {
				// 			"title": this.discussList[i].title,
				// 			"id": this.discussList[i].id,
				// 			"color": colorArray[i]
				// 		}
				// 		result.push(obj)
				// 	}
